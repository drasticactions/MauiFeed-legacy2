...
public class MainToolbarDelegate : AppKit.NSToolbarDelegate
{
    ...
    private SidebarViewController controller;

    /// <summary>
    /// Initializes a new instance of the <see cref="MainToolbarDelegate"/> class.
    /// </summary>
    /// <param name="controller">Root View Controller.</param>
    public MainToolbarDelegate(SidebarViewController controller, IServiceProvider services)
    {
    ...
    public override NSToolbarItem? WillInsertItem(NSToolbar toolbar, string itemIdentifier, bool willBeInserted)
    {
        if (itemIdentifier == Plus)
        {
            ...
            addButton.ItemMenu = UIMenu.Create(
                new[]
                {
                    UIAction.Create(Common.AddFeedButton, UIImage.GetSystemImage("newspaper"), null, action =>
                    {
                        this.controller.AddOrUpdateFeed("https://ascii.jp/biz/rss.xml").FireAndForgetSafeAsync();
                    }),
                    UIAction.Create(Common.AddFolderLabel, UIImage.GetSystemImage("folder.badge.plus"), null,
                        action =>
                        {
    ...
    {
        await this.controller.RefreshFeed();
    }

    [Export("plusClickAction:")]
    public async void PlusClickAction(NSObject sender)
    {
        // Test Feed Add
        // var test = await this.cache.RetrieveFeedAsync("https://ascii.jp/rss.xml");
        // this.db.FeedFolder!.Add(new FeedFolder() { Name = "Foobar" });
        // await this.db.SaveChangesAsync();
        await this.controller.AddOrUpdateFeed("https://ascii.jp/biz/rss.xml");
    }
}